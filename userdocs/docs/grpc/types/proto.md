...
| interfaces | [NetworkInterface](#flintlock.types.NetworkInterface) | repeated | Interfaces specifies the network interfaces to be attached to the microvm. Device names on the guest machine are determines by the order defined in the list starting from eth1, eth2, ..., ethN. |
...
| Field | Type | Label | Description |
| ----- | ---- | ----- | ----------- |
| device_id | [string](#string) |  | DeviceID is the ID of the interface. There is no relation between the ID and the name of the iterface device on the quest machine. |
