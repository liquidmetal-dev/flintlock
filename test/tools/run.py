...
def run_e2e(config_file, org_id, project_name, device_name, skip_teardown):
    ...
    if cfg['device']['id'] is None:
        click.echo(
            f"Running e2e tests. Will create project `{cfg['project_name']}`, ssh_key `{cfg['device']['ssh']['name']}` and device `{cfg['device']['name']}`")
        click.echo(
            "Note: this will create and bootstrap a new device in Equinix and may take some time")
    else:
        click.echo(f"running e2e tests using device `{cfg['device']['id']}`")
    ...
    key_dir = runner.welder.key_dir
    if cfg['test']['skip_teardown']:
        dev_id, dev_ip = runner.device_details()
        click.echo(f"Device `{dev_id}` left alive for debugging. Use with `--config-file` setting `device.id` to re-run tests. SSH command `ssh -i {key_dir}/private.key root@{dev_ip}`. Teardown device with `delete-device` command.")
...
def create_device(config_file, org_id, project_id, device_name, userdata):
    token = os.environ.get("METAL_AUTH_TOKEN")
    if token is None:
        click.echo("must set METAL_AUTH_TOKEN")
        sys.exit()
    ...
    cfg.set_create_flag_config(org_id, project_id, device_name)
    try:
        cfg.validate_create()
    except ValueError as e:
        click.echo(str(e))
