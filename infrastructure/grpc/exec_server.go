...
func (s *execServer) ExecCommand(stream mvmexecv1.MicroVMExec_ExecCommandServer) error {
	...
	// session.Next() below blocks on a socket read with no deadline; if the
	// RPC context ends (client disconnect, deadline, server shutdown) close
	// the session to unblock it instead of leaving the handler stuck until
	// the guest happens to send something.
	go func() {
		<-ctx.Done()
		session.Close()
	}()
...
// state where dialling its guest-agent vsock device makes sense.
