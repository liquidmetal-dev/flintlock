...
// fakeGuestAgentSSHDelayedEcho listens on a UDS, performs the CONNECT
// handshake once, then fully drains its read side to EOF (including a
// client half-close) before writing anything back - so any response it
// sends can only arrive after the client has already signalled it is done
// sending.
func fakeGuestAgentSSHDelayedEcho(t *testing.T, port uint32) string {
	t.Helper()
...
func TestSSHProxyServer_SSHProxy_PreservesResponseAfterClientEOF(t *testing.T) {
	RegisterTestingT(t)

	const sshPort = 1025

	udsPath := fakeGuestAgentSSHDelayedEcho(t, sshPort)
	...
	stream.pushRecv(&mvmsshproxyv1.SSHProxyRequest{
		Payload: &mvmsshproxyv1.SSHProxyRequest_Data{Data: []byte("hello")},
	})
	// Signal client EOF straight away - before the guest has had any chance
	// to respond, unlike TestSSHProxyServer_SSHProxy_EchoRoundtripThenClientEOF.
	...
	case <-time.After(5 * time.Second):
		t.Fatal("SSHProxy did not return")
	}
