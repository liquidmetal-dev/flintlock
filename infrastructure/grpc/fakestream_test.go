...
// withContext overrides the context returned by Context, e.g. so a test can
// cancel it while a handler is in flight. Call before the stream is handed
// to a handler.
func (f *fakeBidiStream[Req, Resp]) withContext(ctx context.Context) *fakeBidiStream[Req, Resp] {
	f.ctx = ctx
