...
func TestExecServer_ExecCommand_ContextCancellationClosesSession(t *testing.T) {
	RegisterTestingT(t)
	...
	// A guest-agent that accepts the connection and then never responds -
	// only the RPC context ending should ever unblock this test.
	udsPath := fakeGuestAgentExec(t, controlPort, func(conn net.Conn) {
		defer conn.Close()
		<-stopServer
	})
	...
	stream := newFakeBidiStream[mvmexecv1.ExecCommandRequest, mvmexecv1.ExecCommandResponse]().withContext(ctx)
	stream.pushRecv(&mvmexecv1.ExecCommandRequest{
		Payload: &mvmexecv1.ExecCommandRequest_Start{Start: &mvmexecv1.ExecStart{Uid: "vm1", Cmd: "sleep", Args: []string{"3600"}}},
	})
	...
	// Give ExecCommand a moment to reach the blocking session.Next() read
	// before cancelling, so this exercises the mid-wait case the bug report
	// described rather than a cancellation racing setup.
	...
	case <-time.After(2 * time.Second):
		t.Fatal("ExecCommand did not return after context cancellation")
	}
}
